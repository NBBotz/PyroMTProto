    ...
    async def _parse(
        ...
        if isinstance(message, raw.types.MessageService):
            ...
            if isinstance(action, raw.types.MessageActionChatAddUser):
                ...
                service_type = enums.MessageServiceType.SUGGESTED_POST_PUBLISHED
        ...
        if isinstance(message, raw.types.Message):
            ...
            # Layer 227: wire suggested_post field
            if getattr(message, "suggested_post", None):
                sp = message.suggested_post
                parsed_message.suggested_post = types.SuggestedPost(
                    accepted=getattr(sp, "accepted", False),
                    rejected=getattr(sp, "rejected", False),
                    price=getattr(sp, "price", None),
                    schedule_date=utils.timestamp_to_datetime(getattr(sp, "schedule_date", None))
                )
            parsed_message.paid_suggested_post_stars = getattr(message, "paid_suggested_post_stars", None)
            parsed_message.paid_suggested_post_ton = getattr(message, "paid_suggested_post_ton", None)
