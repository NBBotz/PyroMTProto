...
class Client(Methods):
    ...
    def __init__(
        ...
        # Per-DC session pools (list of sessions) for parallel downloads
        self.media_sessions: dict[int, list[Session]] = {}
        self.media_sessions_lock = asyncio.Lock()
        # Per-DC creation locks to prevent duplicate session creation race
        self._dc_session_create_locks: dict[int, asyncio.Lock] = {}
        # CDN session pool
        self.cdn_sessions: dict[int, Session] = {}
    ...
    DOWNLOAD_CHUNK_SIZE = 1024 * 1024          # 1 MB per chunk (Telegram max)
    DOWNLOAD_CONNECTIONS_PER_DC = 4            # parallel connections per DC

    async def _get_media_session_pool(self, dc_id: int, n: int) -> list:
        """Get or create a pool of n media sessions for a given DC.

        Uses per-DC locks (optimized) to prevent race conditions while
        allowing different DCs to build their pools simultaneously.
        """
        lock = self._dc_session_create_locks.setdefault(dc_id, asyncio.Lock())
        async with lock:
    ...
    async def _make_media_session(self, dc_id: int) -> "Session":
        """Create, auth and start a single media session for dc_id."""
        ...
        session = Session(self, dc_id, auth_key, test_mode, is_media=True)
        await session.start()
    ...
    async def _get_or_create_media_session(self, dc_id: int) -> Session:
        """Get an existing media session for a DC, or create one with proper locking."""
        # Fast path: session already exists
        sessions = self.media_sessions.get(dc_id)
        if sessions:
            return sessions[0]  # return first for sequential use; caller picks via index

        # Slow path: need to create — use per-DC lock to prevent races
        if dc_id not in self._dc_session_create_locks:
            async with self.media_sessions_lock:
                if dc_id not in self._dc_session_create_locks:
                    self._dc_session_create_locks[dc_id] = asyncio.Lock()

        async with self._dc_session_create_locks[dc_id]:
            # Re-check after lock acquisition
            sessions = self.media_sessions.get(dc_id)
            if sessions:
                return sessions[0]

            # Determine auth key for this DC
            own_dc_id = await self.storage.dc_id()
            test_mode = await self.storage.test_mode()
            if dc_id == own_dc_id:
                auth_key = await self.storage.auth_key()
            ...
            # Create a pool of sessions for this DC
            pool = [
                Session(self, dc_id, auth_key, test_mode, is_media=True)
                for _ in range(self.DOWNLOAD_CONNECTIONS_PER_DC)
            ]

            # Start all sessions in parallel
            await asyncio.gather(*[s.start() for s in pool])

            # If foreign DC, import authorization on all sessions
            if dc_id != own_dc_id:
                for attempt in range(3):
                    try:
                        exported_auth = await self.invoke(
                        ...
                        await asyncio.gather(*[
                            s.invoke(raw.functions.auth.ImportAuthorization(
                                id=exported_auth.id,
                                bytes=exported_auth.bytes
                            ))
                            for s in pool
    ...
    async def get_file(
        ...
        async with self.get_file_semaphore:
            ...
            try:
                # Get session pool for this DC (with race-safe creation)
                await self._get_or_create_media_session(dc_id)
                pool = self.media_sessions[dc_id]

                # For small files or CDN: use standard sequential download
                # For larger files: use parallel chunk fetching
                use_parallel = (
                    file_size > chunk_size * 2
                    and not file_id.file_type == FileType.CHAT_PHOTO
                )

                async def fetch_chunk(session: Session, off: int) -> bytes | None:
                    """Fetch a single chunk at byte offset `off`."""
                    ...
                    if isinstance(r, raw.types.upload.FileCdnRedirect):
                        # Handle CDN redirect (rare but possible)
                        return await self._fetch_cdn_chunk(r, off, chunk_size)
                    return None

                if use_parallel and len(pool) > 1:
                    # Parallel download: pre-fetch CONNECTIONS chunks at a time
                    CONNS = len(pool)
                    current_chunk = 0
                    bytes_downloaded = 0

                    while current_chunk < total_chunks:
                        # Calculate batch of offsets
                        batch_size = min(CONNS, total_chunks - current_chunk)
                        offsets = [
                            offset_bytes + (current_chunk + i) * chunk_size
                            for i in range(batch_size)
                        ]

                        # Fetch all in parallel
                        results = await asyncio.gather(*[
                            fetch_chunk(pool[i % CONNS], off)
                        ...
                        for chunk in results:
                            ...
                            if progress:
                                func = functools.partial(
                                    progress,
                                    min(bytes_downloaded, file_size) if file_size else bytes_downloaded,
                                    file_size,
                                    *progress_args
                                )
                    ...
                    # Sequential download for small files / CDN
                    session = pool[0]
                    current = 0
                    bytes_downloaded = 0
                    ...
                    if isinstance(r, raw.types.upload.File):
                        while True:
                            ...
                            if len(chunk) < chunk_size or current >= total_chunks:
                                break
                        ...
                        async for chunk in self._stream_cdn(r, session, offset_bytes, chunk_size, total_chunks, file_size, progress, progress_args):
                            yield chunk
    ...
    async def _fetch_cdn_chunk(self, redirect, offset_bytes: int, chunk_size: int) -> bytes:
        """Fetch and decrypt a single CDN chunk."""
        cdn_dc = redirect.dc_id
        if cdn_dc not in self.cdn_sessions:
            cdn_session = Session(
                self, cdn_dc,
                await Auth(self, cdn_dc, await self.storage.test_mode()).create(),
                await self.storage.test_mode(), is_media=True, is_cdn=True
            )
            await cdn_session.start()
            self.cdn_sessions[cdn_dc] = cdn_session

        session = self.cdn_sessions[cdn_dc]
        ...
        return aes.ctr256_decrypt(
            ...
            bytearray(redirect.encryption_iv[:-4] + (offset_bytes // 16).to_bytes(4, "big"))
        )

    async def _stream_cdn(self, redirect, origin_session, offset_bytes, chunk_size, total, file_size, progress, progress_args):
        """Stream all chunks from a CDN redirect sequentially."""
        cdn_dc = redirect.dc_id
        if cdn_dc not in self.cdn_sessions:
            cdn_session = Session(
                self, cdn_dc,
        ...
        cdn_session = self.cdn_sessions[cdn_dc]
        current = 0
        bytes_downloaded = 0
        ...
        while True:
            ...
            chunk = r2.bytes
            decrypted = aes.ctr256_decrypt(
                chunk,
                redirect.encryption_key,
                bytearray(redirect.encryption_iv[:-4] + (offset_bytes // 16).to_bytes(4, "big"))
            )

            hashes = await origin_session.invoke(
            ...
            for i, h in enumerate(hashes):
                cdn_chunk = decrypted[h.limit * i: h.limit * (i + 1)]
